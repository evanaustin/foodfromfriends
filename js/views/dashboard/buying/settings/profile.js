		...
		callback: function(result) {
			if (result === true) {
				if ($('div.image-box').hasClass('existing-image')) {
					App.Util.loading();
                    App.Image.destroy('dashboard/buying/settings/remove-image');
...
$('#edit-profile').on('submit', function(e) {
	...
	if ($form.parsley().isValid()) {
		...
		App.Ajax.postFiles('dashboard/buying/settings/save-profile', data, 
			function(response) {
                console.log('js');
                App.Util.finishedLoading();
                App.Util.msg('Your buyer profile has been updated', 'success');
			},
			function(response) {
                App.Util.finishedLoading();
