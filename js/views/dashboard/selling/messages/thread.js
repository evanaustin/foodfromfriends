App.Dashboard.Message = function() {
    function listener() {
        ...
        $('#new-message form').on('submit', function(ev) {
            ...
            App.Ajax.post('dashboard/selling/messages/send', data, 
                function(response) {
                        ...
                        '<div class="col-md-9 offset-md-3">' +
                            '<fable class="margin-btm-50em">' +
                                ...
                                '<cell class="justify-center flexcenter flexgrow-0 margin-left-1em d-none d-md-block">' +
                                    '<div class="user-photo no-margin" style="background-image: url(\'https://s3.amazonaws.com/foodfromfriends/' + ENV + '/grower-operation-images/' + formdata.filename + '.' + formdata.fileext + '\');"></div>' +
                                '</cell>' +
                            '</fable>' +
                        '</div>' +
                    '</div>');
