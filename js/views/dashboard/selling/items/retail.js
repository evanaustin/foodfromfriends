App.Dashboard.RetailItems = function() {
    function listener() {
        ...
        $('#copy-to-wholesale').on('click', function(e) {
            e.preventDefault();
            
            App.Ajax.post('dashboard/selling/items/copy-to-wholesale', $('#edit-items').serialize(),
                function(response) {
                    App.Util.msg('Your items have been been copied to wholesale. Click <a href="' + PUBLIC_ROOT + 'dashboard/selling/items/wholesale">here</a> to view them.', 'success');
                },
                function(response) {
                    App.Util.msg(response.error, 'danger');
                }
